...
class TestResultDataModel(object):
    ...
    def print_results(self, res):
        ...
        for idx, row in enumerate(res.bindings):
            print("Item %d" % idx)
            rowfmt = [f'{key}-->{val.decode()}' for key, val in sorted(row.items())]
            print('\n'.join(rowfmt))
    ...
    def _replace_match(self, graph1, graph2, rdf_type):
        ...
        for g1_term in g1_terms:
            ...
            for p, o in graph1.predicate_objects(g1_term):
                logging.debug(
                    f"Trying to find a match for {str(graph1.qname(g1_term))} {str(graph1.qname(p))} {str(o)}"
                )
                ...
                for g2_term in (x for x in graph2.subjects(p, o) if x in g2_match):
                    g2_match[g2_term] += 1
                    logging.debug(f"Match found with {str(graph1.qname(g2_term))}")
                ...
                if max(g2_match.values()) >= min_matching:
                    if (match_index == max(g2_match.values())) \
                        ...
                        if g1_term != g2_term:
                            ...
                            new_id = f'{g1_term}_{g2_name}'
                            logging.debug(
                                f"{graph1.qname(g1_term)} is matched to {graph2.qname(g2_term)} and replaced by {graph2.qname(new_id)} (match={str(match_index)})"
                            )
                            ...
                            logging.debug(
                                f"{graph1.qname(g1_term)} is matched to {graph2.qname(g2_term)} (match={str(match_index)})"
                            )
            ...
            if not match_found:
                logging.debug(f"No match found for {graph1.qname(g1_term)}")
